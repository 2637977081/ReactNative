import React, { Component } from 'react';
import {
    Text,
    View,
    Image,
    TouchableOpacity,
    StyleSheet,
    Dimensions,
} from 'react-native';
import BookDetail from './bookDetail';

let ScreenWidth = Dimensions.get("window").width;
let ScreenHeight = Dimensions.get("window").height;

export default class BookBarItem extends Component {

    constructor(props) {
        super(props);
        this.state = {
            item: null,
        }
        console.log("Item显示" + this.props.item);
    }

    render() {
        return (
            <TouchableOpacity
                onPress={() => {
                    this.props.navigation.navigate('BookDetail', this.props.item);
                }}

                style={styles.btn}
            >
                <View style={styles.main}>
                    <View>
                        <Image
                            source={{ uri: this.props.item.bookImage }}
                            style={{ width: ScreenWidth*0.25, height: ScreenWidth*0.25+30, marginLeft: 10, }}
                        />
                    </View>

                    <View style={{ flex: 1,  marginLeft: 20 }}>
                        <View style={styles.textView}>
                            <Text numberOfLines={1} style={{color:'#000',fontWeight:'bold',fontSize:16,}}>
                                {this.props.item.bookTitle}
                            </Text>
                        </View>
                        <View style={styles.textView}>
                            <Text>
                                {this.props.item.bookPublisher}
                            </Text>
                        </View>
                        <View style={styles.textView}>
                            <Text>
                                {this.props.item.bookAuthor}
                            </Text>
                        </View>
                        <View style={{flexDirection:'row',flex:1,alignItems:'center',marginBottom:5,}}>
                            <Text style={{color:'#4a9692',fontSize:16,}}>
                                {this.props.item.bookPrice}
                            </Text>
                            <Text style={{marginLeft:15}}>
                                {this.props.item.bookPages}页
                            </Text>
                        </View>

                    </View>
                </View>

            </TouchableOpacity>
        )

    }


}

const styles=StyleSheet.create({
    btn:{
        backgroundColor: '#fff',
        height: ScreenWidth*0.25+40,
    },
    main:{
        marginTop:10,
        flexDirection: 'row',
    },

    textView:{
        marginTop:6,
    },
});